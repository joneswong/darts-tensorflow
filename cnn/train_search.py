...
def main():
    ...
    if args.unrolled:
        ...
        leader_opt = compute_rolled_step(x_valid, y_valid)
...
def compute_rolled_step(x_valid, y_valid):
    ...
    arch_regularization_loss = tf.add_n(
        utils.get_var(tf.losses.get_regularization_losses(), 'arch_var')[1])
    valid_loss += (1e4 * args.arch_weight_decay) * arch_regularization_loss
    leader_opt = leader_opt.minimize(valid_loss, var_list=arch_var)
    return leader_opt
...
def _pre_process(x, label):
    ...
    if cutout_length is not None:
        ...
        mask = mask[cutout_length:cutout_length +
                    32, cutout_length:cutout_length + 32]
