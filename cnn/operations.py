...
OPS = {
    'none':
    lambda x, C, stride: Zero(x, stride),
    'avg_pool_3x3':
    lambda x, C, stride: slim.avg_pool2d(
        x, [3, 3], stride=stride, padding='SAME'),
    'max_pool_3x3':
    lambda x, C, stride: slim.max_pool2d(
        x, [3, 3], stride=stride, padding='SAME'),
    'skip_connect':
    lambda x, C, stride: tf.identity(x)
    if stride == [1, 1] else FactorizedReduce(x, C),
    'sep_conv_3x3':
    lambda x, C, stride: SepConv(x, C, [3, 3], stride),
    'sep_conv_5x5':
    lambda x, C, stride: SepConv(x, C, [5, 5], stride),
    'sep_conv_7x7':
    lambda x, C, stride: SepConv(x, C, [7, 7], stride),
    'dil_conv_3x3':
    lambda x, C, stride: DilConv(x, C, [3, 3], stride, 2),
    'dil_conv_5x5':
    lambda x, C, stride: DilConv(x, C, [5, 5], stride, 2),
