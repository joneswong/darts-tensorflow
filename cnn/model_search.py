...
def Cell(s0, s1, cells_num, multiplier, C_out, reduction, reduction_prev):
    ...
    for i in range(cells_num):
        ...
        for j in range(2 + i):
            ...
            temp.append(MixedOp(state[j], C_out, stride, offset + j,
                                reduction))
        offset += len(state)
...
def Model(x,
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        with slim.arg_scope([slim.conv2d, slim.separable_conv2d],
            ...
            with slim.arg_scope([slim.batch_norm], is_training=is_training, center=False, epsilon=1e-5, decay=0.9):
                # for the k-th cell, s1, s0 are the outputs of the (k-1)-th and (k-2)-th cell respectively
                C_curr = stem_multiplier * first_C
                s0 = slim.conv2d(x, C_curr, [3, 3])
                s0 = slim.batch_norm(s0, center=True, scale=True)
                s1 = s0
                ...
                for i in range(layer_num):
                    if i in [layer_num // 3, 2 * layer_num // 3]:
                        ...
                        reduction = False
                    s0, s1 = s1, Cell(s0, s1, cells_num, multiplier, C_curr,
                                      reduction, reduction_prev)
                    reduction_prev = reduction
                out = tf.reduce_mean(s1, [1, 2],
                                     name='global_pool')
